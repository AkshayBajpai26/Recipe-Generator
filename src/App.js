...
function App() {
    ...
    const generateRecipes = (ingredients) => {
        // This is a simplified recipe matching algorithm
        // In a real app, this would use AI/ML services
        const ingredientNames = ingredients.map((i) => i.name.toLowerCase());
        return sampleRecipes.filter(recipe => {
            const recipeIngredients = recipe.ingredients.map((i) => i.name.toLowerCase());
            const matchCount = ingredientNames.filter(name => recipeIngredients.some((recipeIngredient) => recipeIngredient.includes(name) || name.includes(recipeIngredient))).length;
            return matchCount >= Math.min(2, ingredients.length);
        });
    };
    return (_jsxs("div", { className: "App", children: [_jsx(Header, {}), _jsxs("main", { className: "main-content", children: [_jsx(IngredientInput, { onSubmit: handleIngredientsSubmit }), loading && (_jsxs("div", { className: "loading", children: [_jsx("div", { className: "spinner" }), _jsx("p", { children: "Finding delicious recipes for you..." }), _jsxs("div", { className: "loading-steps", children: [_jsxs("div", { className: `loading-step ${loadingStep >= 1 ? 'completed' : ''}`, children: [_jsx("div", { className: `step-icon ${loadingStep === 1 ? 'active' : loadingStep > 1 ? 'completed' : 'pending'}`, children: loadingStep > 1 ? '✓' : loadingStep === 1 ? '⟳' : '1' }), _jsx("span", { children: "Analyzing your ingredients" })] }), _jsxs("div", { className: `loading-step ${loadingStep >= 2 ? 'completed' : ''}`, children: [_jsx("div", { className: `step-icon ${loadingStep === 2 ? 'active' : loadingStep > 2 ? 'completed' : 'pending'}`, children: loadingStep > 2 ? '✓' : loadingStep === 2 ? '⟳' : '2' }), _jsx("span", { children: "Searching recipe database" })] }), _jsxs("div", { className: `loading-step ${loadingStep >= 3 ? 'completed' : ''}`, children: [_jsx("div", { className: `step-icon ${loadingStep === 3 ? 'active' : loadingStep > 3 ? 'completed' : 'pending'}`, children: loadingStep > 3 ? '✓' : loadingStep === 3 ? '⟳' : '3' }), _jsx("span", { children: "Calculating perfect matches" })] })] })] })), !loading && recipes.length > 0 && (_jsx(RecipeList, { recipes: recipes, onRecipeSelect: setSelectedRecipe, ingredients: ingredients })), selectedRecipe && (_jsx(RecipeDetail, { recipe: selectedRecipe, onClose: () => setSelectedRecipe(null) }))] })] }));
}
// Sample recipe data
const sampleRecipes = [
    {
