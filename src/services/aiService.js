class AIService {
    ...
    async recognizeIngredientsLocal(imageFile) {
    ...
    async recognizeIngredientsCloud(imageFile) {
        const startTime = Date.now();
        try {
    ...
    async recognizeWithChatGPT(imageFile) {
        const startTime = Date.now();
    ...
    async recognizeWithHuggingFace(imageFile) {
    ...
    async recognizeWithImagga(imageFile) {
        const startTime = Date.now();
        // Convert image to base64
        const base64Image = await this.fileToBase64(imageFile);
        ...
        try {
            ...
            const ingredients = data.result.tags
                ...
                return item.confidence > 30 && foodKeywords.some(keyword => tagName.includes(keyword) || keyword.includes(tagName));
            })
                .map((item) => ({
    ...
    async generateWithOpenAI(ingredients) {
    ...
    generateLocalSuggestions(ingredients) {
